...
const CategoryForm = ({ onSubmit, newCat, setNewCat, categories }) => (
    <form onSubmit={onSubmit} className="bg-card p-6 rounded-[2rem] border border-border shadow-sm space-y-4">
        <input 
            type="text" 
...
const CategoryList = ({ categories, onViewCategory }) => (
    <div className="space-y-3">
        {categories.filter(c => !c.parentCategoryId).map(parent => (
            <div 
                key={parent.categoryId} 
                onClick={() => onViewCategory(parent)} 
...
// ✅ SUB-COMPONENT: TAG SECTION
const TagSection = ({ tags, onAddTag, newTag, setNewTag }) => (
    <div className="space-y-6">
        ...
        <form onSubmit={onAddTag} className="flex gap-3">
            <input 
                type="text" 
                placeholder="#newtag" 
                aria-label="Tag Name"
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10 animate-in fade-in duration-700 transition-colors duration-300">
            <div className="space-y-6">
                ...
                <CategoryList 
                    categories={categories} 
                    onViewCategory={setSelectedViewCategory} 
                />
            </div>
