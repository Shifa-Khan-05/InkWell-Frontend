...
const Dashboard = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center space-y-4">
        ...
        <div className="text-blue-500 font-mono text-xs tracking-widest uppercase">Establishing Secure Connection...</div>
    </div>
  );
      ...
      <div className="flex flex-1 relative">
        ...
        <aside className={`fixed md:static z-50 top-0 left-0 h-full w-64 bg-gray-950 border-r border-gray-800 transform transition-transform duration-300 flex flex-col ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 shadow-2xl`}>
          <div 
            ...
            className="p-6 text-2xl font-black border-b border-gray-800 bg-gradient-to-r from-blue-500 to-purple-500 bg-clip-text text-transparent italic flex justify-between items-center font-serif cursor-pointer hover:opacity-80 transition"
          >
            InkWell
          ...
          <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
            <button onClick={() => navigate('/browse')} className="w-full flex items-center gap-3 p-3 rounded-xl text-gray-400 hover:text-white hover:bg-gray-900 transition">
            ...
            {/* ✅ SYSTEM ADMIN SECTION - RESTORED & IMPROVED */}
            {role === 'ADMIN' && (
          ...
          <header className="flex justify-between items-center px-8 py-5 border-b border-gray-800 bg-black/50 backdrop-blur-md sticky top-0 z-40">
            ...
            <div className="flex items-center gap-3 bg-gray-900 border border-gray-800 p-1.5 pr-5 rounded-full">
              <div className="h-9 w-9 rounded-full overflow-hidden border-2 border-blue-500/20">
                <img src={userData.profileImageUrl || `https://ui-avatars.com/api/?name=${userData.fullName || 'User'}&background=0D8ABC&color=fff`} className="w-full h-full object-cover" alt="Avatar" />
              </div>
              <span className="text-white text-sm font-bold truncate max-w-[150px] uppercase tracking-tighter">{userData.fullName || 'User Identity'}</span>
            </div>
          </header>

          <main className="p-8 overflow-y-auto flex-1 bg-[#050505]">{renderContent()}</main>
        </div>
      </div>
    </div>
  );
